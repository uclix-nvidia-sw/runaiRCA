...
| `agent.env.runaiBaseUrl` / `runaiTokenUrl`     | Run:ai API URL; the token endpoint is auto-derived from it (documented `/api/v1/token` first) — set `runaiTokenUrl` only as an override for unusual deployments |
