...
| Collector | Owns |
|---|---|
| **runai** | Run:ai API 워크로드/프로젝트/큐/쿼터/버전 컨텍스트(선택적으로 [runai-mcp 서비스](#run-ai-mcp-service), 426개 API 경유) |
...
[runai-mcp](https://github.com/sejongjeong/runai-mcp) 서버(mcp-proxy 뒤 ClusterIP 서비스로 배포)에
접근하여 426개 Run:ai API에 스펙 인지 방식으로 접근합니다. Helm 차트는 기본적으로 서비스를
실행하고 이 URL을 설정합니다(`runaiMcp.enabled: true`). 어떤 실패든 고정 엔드포인트 직접-HTTP
...
`DRILLDOWN_MAX_STEPS`, `RUNAI_DB_DSN`, `ANALYSIS_DEADLINE_SECONDS`,
