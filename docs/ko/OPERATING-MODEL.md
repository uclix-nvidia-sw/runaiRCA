...
명시적으로 요청하는 채팅 요청(RCA 버튼)에서 수동으로 분석 실행을 생성할 수도 있습니다.
이 버튼은 먼저 UI에서 이미 선택된 알림이나 인시던트를 그대로 대상으로 삼습니다. 명시적으로
선택된 대상이 없으면, 채팅 메시지 자체가 기존 알림이나 인시던트를 언급할 때만("알람"/"최근"
같은 한국어 또는 "alert"/"latest" 같은 영어 키워드) 가장 최근의 미해결 알림을 대상으로
삼고, 그렇지 않으면 메시지 내용으로 새 ad-hoc 인시던트를 만들어 그것을 분석합니다.
Analysis Dashboard는 `/api/v1/analysis-runs`를 기반으로 합니다. 이 목록이 비어 있다면,
아직 어떤 분석 트리거도 Backend에 도달하지 않은 것입니다.
...
쿨다운만이 관문은 아닙니다. 자동 재분석은 웹훅이 **새 정보**를 담고 있을 것도 요구합니다.
pod이 삭제되지 않는 알림은 영영 resolve되지 않으므로 Alertmanager가 `repeat_interval`마다
동일한 fingerprint와 `StartsAt`으로 재전송하는데, 이는 같은 에피소드라 쿨다운이 아무리
지나도 재분석하지 않습니다. 최초 인지, resolve→firing, 새 `StartsAt`, severity 상승,
occurrence 증가는 여전히 새 정보로 인정됩니다. 이는 낭비되는 실행만의 문제가 아닙니다 —
해시가 달라진 재분석이 완료되면 운영자의 승인이 해제되고 그로부터 파생된 evaluation
리뷰와 지식이 삭제되므로, 지우지 않은 테스트 pod 하나가 타이머처럼 승인된 RCA를
지워버립니다. 실제로 *실패한* 분석의 재시도는 영향을 받지 않습니다 — 완료된 RCA가 없는
알림은 백필 루프가 자체 쿨다운으로 다시 처리합니다.
...
- Chat Agent는 읽기 전용 cross-domain 드릴다운 도구 위에서 에이전트 루프를 실행하고,
  ...
  인시던트/알림 대상으로 범위가 한정되며, frontend context picker로는 대화를 인시던트
  하나에 고정할 수만 있습니다 — 전체 클러스터 옵션은 없습니다. 일반 Send는 이 인시던트/알림
  컨텍스트를 요구하며, 새 질문은 대신 RCA 버튼으로 던져야 합니다. RCA 버튼은 chat-adhoc
  인시던트를 만들고 전체 분석을 실행합니다. 그 분석이 클러스터에서 일치하는 증거를 찾지
  못하면, 보고서는 빈 insufficient-evidence stub이 아니라 결정론적 지식 베이스 답변
  (`answer_mode="knowledge_only"`, UI 배너로 표시)이 됩니다. 이런 run은 유사 인시던트
  메모리나 승격된 지식이 되지 않습니다. 결정론적 컨텍스트 기반 답변은 그 외의 일반 채팅
  응답에서는 chat LLM이 구성되지 않았을 때만 사용하는 폴백입니다.
