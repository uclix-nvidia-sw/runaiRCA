...
Postgres, Change, System의 발생 증거는 해결 시각에 끝나는 인과 시간 창 안에서만 승격됩니다.
...
- Chat Agent는 활성 RCA, 알림 분석, 증거 추적, 피드백, 유사 인시던트 메모리로부터
  운영자의 후속 질문에 답합니다. 현재 구현에서 `/chat`은 결정론적이며 컨텍스트에 기반합니다.
  NeMo/LLM 런타임을 직접 호출하지 않습니다. `ENABLE_NAT_RUNTIME=true`는 `/analyze` 합성에
  영향을 줍니다. 첨부된 인시던트나 알림 RCA 내용 없이 대시보드 페이지에서 채팅이 열리면,
  Backend는 대시보드 및 분석 실행 상태를 첨부하여 Chat이 현재 알림 수, 최신 실행 상태,
  에이전트 타임아웃/실패 경고, 데이터베이스 상태, 런타임 모드를 보고할 수 있도록 합니다.
