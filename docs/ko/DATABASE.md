...
그래프는 Postgres로부터 **파생**됩니다 — 리뷰 게이트가 적용된 투영이며, 두 번째 신뢰의
원천이 아닙니다.
...
| ✅ 지식 (`load_knowledge` / `load_xids` / `load_alerts` / `load_known_issues` / `load_architecture`) | `symptom`/`root_cause`/`action` + `indicates`/`resolved_by`, `xid_error` + `leads_to`, `control_plane_component` + `depends_on` |
| 🟦 승격됨 (`ingest.py --promote-knowledge`) | 운영자가 확인한 RCA로부터 `confirmed:<alert>` 증상 → 패밀리 → 조치 |
| ⬜ 모델링됨, 아직 미공급 | `evidence`, `runbook`, `analysis_run`, `similar_to`, `supported_by`, GPU 속성 |
...
| Curated knowledge | `load_knowledge`, `load_xids`, `load_alerts`, `load_known_issues`, `load_architecture` | `knowledge/` 카탈로그들 | 버전 관리되는 파일, 스키마 잡에서 실행 |
| Topology + incidents | `ontology/ingest.py` (CronJob) | Postgres `incidents`/`alerts` | `resolvedGraceHours` 이전에 resolved됨; `requireReview=false`가 아니면 리뷰 게이트 적용 |
