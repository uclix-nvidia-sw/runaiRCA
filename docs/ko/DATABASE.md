...
투영입니다. 기본적으로 grace period가 지난 resolved 인시던트를 담으며, 운영자 승인을
추가 게이트로 쓰려면 `typedb.ingest.requireReview=true`로 설정합니다.
...
| Topology + incidents | `ontology/ingest.py` (CronJob) | Postgres `incidents`/`alerts` | resolved 후 `resolvedGraceHours` 이상 경과. 기본 `requireReview=false`에서는 승인 여부와 관계없이 적격 인시던트 포함, `true`이면 `user_approved_at` 필요 |
