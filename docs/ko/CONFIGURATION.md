...
| `RUNAI_BASE_URL` | Run:ai 컨트롤 플레인 URL. 기본값 `http://runai-backend-traefik.runai-backend.svc.cluster.local` |
...
| `agent.env.runaiBaseUrl` / `agent.env.runaiTokenUrl` | Run:ai API와 선택 사항인 OAuth 토큰 엔드포인트. 기본 Run:ai API는 `http://runai-backend-traefik.runai-backend.svc.cluster.local`입니다. Run:ai HTTP 401을 방지하려면 `secrets.runaiBearerToken` 또는 클라이언트 자격 증명을 제공하세요 |
