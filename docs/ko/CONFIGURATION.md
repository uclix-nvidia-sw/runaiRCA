...
| `KUBERNETES_MCP_URL` | Kubernetes MCP shared service URL. 설정하면 Kubernetes 수집기와 드릴다운이 MCP를 먼저 호출하고, 실패 시 기존 Kubernetes API 직접 읽기로 폴백합니다 |
...
| `RUNAI_MCP_URL` | runai-mcp 사이드카 URL(stdio→HTTP 브리지, 예: `http://localhost:8809/mcp`). 설정하면 runai 수집기와 드릴다운이 426개 Run:ai API를 스펙 인식 방식으로 접근합니다. 실패 시에는 고정 엔드포인트 수집기로 폴백합니다. Helm은 기본적으로 사이드카를 실행하고 이 값을 설정합니다(`runaiMcp.enabled: true`) |
...
| `COLLECTORS` | 쉼표로 구분된 수집기 레지스트리 allowlist. 비어 있거나 기본값이면 모든 내장 수집기를 활성화합니다 |
...
| `LLM_MODEL_PLANNER` / `LLM_MODEL_INVESTIGATION` / `LLM_MODEL_DRILLDOWN` / `LLM_MODEL_SELF_CHECK` / `LLM_MODEL_SYNTHESIS` / `LLM_MODEL_CHAT` | 단계별 모델 override. 비어 있으면 `LLM_MODEL`로 폴백합니다 |
...
| `ENABLE_NAT_RUNTIME` | NeMo Agent Toolkit CLI를 통해 RCA 종합을 실행합니다. 기본값 `true` |
| `NAT_CONFIG_FILE` | NeMo 워크플로 구성 경로. 기본값 `configs/runai_rca_workflow_litellm.yml` |
...
| `ANALYSIS_TOKEN_BUDGET` | 분석당 선택 사항인 LLM 토큰 예산. `0`은 무제한이며, 초과 시 investigation/drill-down이 다음 안전한 step 경계에서 멈추고 warning을 기록합니다 |
...
- `agent/configs/runai_rca_workflow_mcp.yml`은 NIM 기반 Analysis Agent 검토 경로를 유지하지만,
  final synthesis에 raw MCP client group을 직접 노출하지 않습니다. MCP 호출은 각 도메인
  수집기/드릴다운 내부에서 먼저 수행되고, final synthesis는 그 결과 artifact만 봅니다.
- `agent/configs/runai_rca_workflow_litellm.yml`은 기본 런타임 워크플로입니다.
  환경 변수나 Helm Secret 값을 통해 `LLM_BASE_URL`, `LLM_MODEL`,
  `LLM_API_KEY`를 제공하세요.
...
| `grafanaMcp.enabled` / `grafanaMcp.grafanaUrl` | Prometheus/Loki datasource tool을 위한 shared Grafana MCP ClusterIP 서비스를 실행합니다. 토큰은 `secrets.existingSecret`의 `GRAFANA_SERVICE_ACCOUNT_TOKEN`을 사용합니다 |
| `kubernetesMcp.enabled` | 자체 읽기 전용 ServiceAccount/RBAC로 shared Kubernetes MCP ClusterIP 서비스를 실행합니다. `secrets`와 `pods/exec` 권한은 부여하지 않습니다 |
| `postgresMcp.enabled` | `runai-rca-postgres-mcp` 래퍼 이미지 기반 shared Postgres MCP ClusterIP 서비스를 실행합니다 |
...
`llm_usage` 같은 값을 담기 위한 `metadata` JSONB가 포함됩니다. 명시적으로 분석을 요청하는 코멘트와 채팅
