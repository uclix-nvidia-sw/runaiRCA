...
`ANALYSIS_DEADLINE_SECONDS`(기본값 1500초)로 제한됩니다. Backend의
`AGENT_REQUEST_TIMEOUT_SECONDS`(1560초)는 이보다 위에 유지되어, 우아하게 저하된 리포트가
...
- 동일 페이지 하단에 에이전트 증거 추적(Agent Evidence Trail)
...
- **스키마**(`agent/ontology/schema.tql`): 타입이 지정된 엔티티(cluster, node, pod, workload,
  ...
  15개의 근본 원인(root cause) 패밀리.
- **수집(Ingestion)**(`agent/ontology/ingest.py`, CronJob): 유예 기간 이후 해결된
  `incidents`/`alerts`를 그래프로 결정론적으로 투영. 선택적으로 리뷰 게이트를 걸 수 있으며
  (`requireReview`), 운영자가 확인한 RCA를 재사용 가능한 지식으로 승격할 수 있습니다
  (`--promote-knowledge`).
- **강화(Enrichment)**(`agent/app/services/kg_enrichment.py`): 오케스트레이터는 평면
  ...
  결코 예외를 발생시키지 않습니다.
- **랭킹**(`agent/app/services/root_cause_ranking.py`)은 실패 패밀리(failure family)에 점수를
  매기고(규칙 R1-R6), **시그니처 승격(signature promotion)**이 가장 구체적인 매칭을 헤드라인으로
  올립니다. 이는 유사도가 아니라 *원인*의 순위를 매깁니다 — "어떤 과거 인시던트가 유사한가"는
