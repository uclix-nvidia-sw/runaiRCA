...
주요 값(전체 시크릿 키: `DATABASE_URL`, `POSTGRES_DSN`, `RUNAI_CLIENT_ID`, `RUNAI_CLIENT_SECRET`, `RUNAI_BEARER_TOKEN`, `NVIDIA_API_KEY`, `LLM_API_KEY`):
...
| `agent.env.runaiBaseUrl` / `runaiTokenUrl`     | Run:ai API URL 및 OAuth 토큰 URL (client\_id/secret 사용 시 토큰 URL 필수)          |
...
| `typedb.ingest.requireApproval`                | Dashboard 승인(`user_approved_at`)을 받은 resolved 인시던트만 그래프에 적재합니다. 기본값 `true`; `requireReview`는 deprecated입니다. |
...
* [조사 기록·온톨로지·학습 지식 안내](LEARNING-AND-ONTOLOGY.md) — probe, diagnosis, trace-v3, 안전한 지식 공개를 처음부터 이해하기
* [Knowledge Base](KNOWLEDGE-BASE.md) — 큐레이션된 카탈로그 + TypeDB 온톨로지
* [Ontology & Ingestion Guide](ONTOLOGY-GUIDE.md) — 승인 RCA 그래프 모델, 데이터 적재 기준, Studio 확인 쿼리
* [Evaluation & Runtime Harness](EVALUATION.md) — 출력 gate, 수정, 운영자 평가
